<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Strategy and skill in web design</title>
    <!-- FONT AWESOME -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css">
    <!-- Styles -->
    <link rel="stylesheet" href="../styles/mains.css">
    <!-- FAVICON -->
    <link rel="shortcut icon" href="../img/Favicon1.png" type="image/x-icon">
    <!-- Oswald -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Oswald:wght@200;300;400&display=swap" rel="stylesheet">
</head>
<body>
    
    <div class="container">
        <nav class="nav-main">
            <img src="../img/logo 4.jpg" alt="logo SSWD" class="nav-brand">
            <ul>
                <li>
                    <a href="../php/index-welcome.php">Inicio</a>
                </li>
                <li>
                    <a href="../php/index-M-V.php">Misión y Vición</a>
                </li>
                <li>
                    <a href="../php/index-informacion.php">Informacion</a>
                </li>
            </ul>
            <ul class="nav-menu-right">
                <li>
                    <a href="../php/perfil_user.php"><i class="fas fa-user"></i></a>
                </li>
            </ul>
        </nav>
    </div>
    <hr>
    <!-- SHOWCASE -->
    
    <header class="showcase">
        <h2>Quienes somos</h2>
        <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Beatae eos laudantium, deserunt incidunt labore reiciendis similique?</p>
    </header>

    <header class="showcase-2">
        <h2>Proyectos</h2>
        <p>Estos son los proyectos elaborados en Strategy and skill in web design</p>
        <a href="#" class="btn">Ver mas<i class="fas fa-angle-double-right"></i></a>
    </header>

</body>
</html>



